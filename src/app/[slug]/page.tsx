...
export default function CommunityDetailPage() {
  ...
  useEffect(() => {
    // 개발 환경 StrictMode의 이펙트 2회 호출 방지 및 슬러그 단위 단발 요청 보장
    if (initRef.current === slug) return
    initRef.current = slug
    loadCommunityData()
  }, [slug])

  // 전역 사용자 상태는 AuthProvider에서 제공
  ...
  const loadCommunityData = async () => {
    try {
      ...
      setMainIdx(0)
      try { const totalStats = await fetchDashboardStats(body.community.id); setStats(totalStats) } catch {}
    } catch (error) {
  ...
  const handleJoinToggle = async () => {
    ...
    try {
      setIsJoining(true)
      if (isMember) {
        // 가입된 커뮤니티 클릭 시 대시보드로 이동
        router.push(`/${community.slug}/dashboard`)
      } else if (isPending) {
        // 승인 대기 취소: pending 행 삭제
                  ...
                  {services.length === 0 ? (
                    ...
                    services.map((s, i) => (
                      <div key={s.id} className="flex items-center gap-2 sm:gap-3">
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${['bg-blue-500','bg-green-500','bg-purple-500','bg-orange-500','bg-teal-500'][i % 5]}`} />
